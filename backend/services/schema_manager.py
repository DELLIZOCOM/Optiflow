...
from backend.config.paths import (
    SCHEMA_CONTEXT_PATH, SCHEMA_INDEX_PATH, TABLES_DIR,
    SCHEMA_META_PATH, SECURITY_PATH, KNOWLEDGE_DIR, COMPANY_MD_PATH,
    DB_CONFIG_PATH, MODEL_CONFIG_PATH, CONFIG_DIR, SUGGESTED_Q_PATH,
)
...
def _write_schema_index(tables_data: list) -> None:
    ...
    index_path = str(SCHEMA_INDEX_PATH)
    lines = []
...
_SCHEMA_META_PATH = str(SCHEMA_META_PATH)
...
def is_setup_complete() -> bool:
    ...
    """
    schema_path = str(SCHEMA_CONTEXT_PATH)
    if not os.path.exists(schema_path):
        return False

    model_cfg_path = str(MODEL_CONFIG_PATH)
...
def save_db_credentials(server: str, database: str, user: str, password: str) -> None:
    ...
    from backend.config.loader import save_db_config
    save_db_config({
        "server":   server,
        "database": database,
        "user":     user,
...
_SECURITY_PATH = str(SECURITY_PATH)
...
def generate_suggested_questions() -> list:
    """Generate 6 suggested questions from schema_index.txt via AI, with fallback."""
    import re
    schema_index_path = str(SCHEMA_INDEX_PATH)
    ...
    try:
        ...
        from backend.ai.prompts import SUGGEST_QUESTIONS_SYSTEM
        text = get_completion(
        ...
        candidate = fence.group(1) if fence else (text[start:end + 1] if start != -1 and end > start else text)
    ...
    return [
        ...
        for i, (lbl, q) in enumerate(tpls)
    ]


def save_suggested_questions(questions: list) -> None:
    try:
        os.makedirs(os.path.dirname(str(SUGGESTED_Q_PATH)), exist_ok=True)
        with open(str(SUGGESTED_Q_PATH), "w", encoding="utf-8") as f:
            json.dump(questions, f, indent=2, ensure_ascii=False)
    except Exception as exc:
        logger.warning(f"Could not save suggested questions: {exc}")


def load_suggested_questions() -> list:
    try:
        if os.path.exists(str(SUGGESTED_Q_PATH)):
            with open(str(SUGGESTED_Q_PATH), encoding="utf-8") as f:
                return json.load(f)
    except Exception:
        pass
    return []
